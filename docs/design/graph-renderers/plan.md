...
|---|---|---|---|
| D1 | 2D vs 3D primacy | **3D primary; ONE 2D lane decided after lab-02/03 measure the label field and compound containment; lab-06 only if that lane is alive** — RULED 2026-09-07 (operator): every 2D option is discarded; lab-02/03 stay as records, lab-06 stays unbuilt | The station's thesis (drawn containment, the hull bleed, the game direction) is 3D and every game host that survives `file://` is 3D; 2D wins on labels and layout semantics — a reading station, not the instrument. |
