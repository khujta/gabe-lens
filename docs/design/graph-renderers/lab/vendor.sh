...
{ echo "["; (IFS=$'\n'; printf '%s\n' "${rows[@]}" | paste -sd, -); echo "]"; } > "$V/MANIFEST.json"
python3 -c "import json;d=json.load(open('$V/MANIFEST.json'));print('MANIFEST.json:',len(d),'bundles,',sum(x['bytes'] for x in d),'bytes')"
