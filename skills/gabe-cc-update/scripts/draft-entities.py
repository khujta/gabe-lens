...
usage: python3 draft-entities.py <project-root> [--out <path>] [--json] [--model proposed|derived] [--min N]
...
def project(models: dict, head: str, model: str, min_endpoints: int, sib) -> dict:
    ...
    for r in rows:
        if r.get("kind", "feature") == "feature" and int(r.get("endpoints") or 0) < min_endpoints:
            continue
        name = r.get("name") or _words(r.get("anchor_table") or r.get("id"))
        row = {"id": r.get("id"), "name": name, "kind": ("candidate feature" if model == "proposed" else (r.get("kind") or "feature")),
               ...
               "suggested_slug": r.get("suggested_slug") or _slug(name if r.get("named_by") == "domain" else (r.get("anchor_table") or name)),
               "action": _action(r.get("members") or [], sib), "why": r.get("why"), "members": (r.get("members") or [])[:40], "members_more": r.get("members_more", 0)}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if a.model == "proposed" and not ((models.get("views") or {}).get("proposed") or {}).get("present"):
        ...
        return 0
    draft = project(models, str(c4.get("head") or ""), a.model, max(1, a.min), _sibling())
    ...
    if a.json:
        ...
        print(f"draft-entities ({a.model}): {len(draft['declared'])} declared verdict(s) [{tally}] · {len(draft['candidates'])} candidate(s) · "
              f"witnessed {cov['anchored']}/{cov['atoms']} atoms · {draft['abstained']['count']} abstained · head {draft['head'] or '?'} → {out}{'' if changed else ' (unchanged)'}")
        ...
        for c in draft["candidates"]:
            print(f"  CANDIDATE {c['name']}  (named by its {c.get('named_by') or 'table'}; slug {c.get('suggested_slug')}; {c.get('endpoints') or 0} endpoint(s))"
