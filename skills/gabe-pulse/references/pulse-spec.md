...
| S17 | homing evidence — pieces whose USERS / DATA witnesses disagree with their FILE (Part C, 2026-09-06: location is an indicator, never the definition), read from the committed c4 `stats.homing`; fires at ≥3 move candidates (≥60% of ≥2 users in ONE other entity, data agrees); the shared count is reported beside it, never a trigger; nothing re-homes | `/gabe-cc-init section` (`mcp__gabe-map__entity_models model=seeded` shows the moves applied) | committed `c4-graph.json` `stats.homing` |
| S18 | entity proposals — the committed `entities.draft.json` (the entity-model drafter's projection of the c4 `models` block: a verdict per declared entity + NAMED candidate entities) carries ≥1 non-FEATURE verdict (SPLIT · MERGE · ASPECT · LAYER) or ≥2 candidates; two states — the draft's `head` equals the committed map's → the REVIEW is owed; heads differ → the draft is STALE and the RUN is owed; no file → silent, unparseable → 0 never a crash; the line names the naming strategy in force and, when the draft carries them, a naming `config_error` and the count of `naming.words` that name nothing (naming-plan Phase 5 — one clause, no new trigger) | `/gabe-cc-init rank` (review) · `draft-entities.py .` (stale) | committed `docs/site/center/entities.draft.json` + the c4 head |
