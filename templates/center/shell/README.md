...
| `gabe-universe.html` (the ENTITY MODEL) | **SETTLED on `seeded`, 2026-09-07** (operator). The station once carried three chooser pills — ENTITY MODEL, NAMES and FRONTEND MARK; all three are gone, with their setters, their `?model=`/`?naming=`/`?feconv=` deep links and their stored preferences. What survives is the DELTA APPLIER `_applyEntityModel`, internal, run ONCE at boot: `window.__uniModel` is only a LABEL and the per-piece home deltas live inside that function, so a station that sets the label without applying them announces "seeded · N moved" over a field that never moved. It reads `window.GABE_C4.models` (+ `GABE_LEVELS.models` for the function layer), re-homes pieces, registers new clusters and re-anchors — node arrays, ids and colours are never touched, so a walk survives. A feed with NO seeded view keeps the claim clustering and the Sources row says so. NAMES went because it could never relabel a claim or a seeded cluster (only a derived/candidate row carries `names{}`); the FRONTEND MARK went because `case` — frontend camelCase, backend PascalCase — is settled. `__uniStrategy()` and `__uniConvention()` survive as the LABEL path and read the feed's own defaults. The four models are unchanged in the DATA: gabe-map `entity_models`, pulse S9/S18, the drafter and `/gabe-cc-init rank` still weigh all four. | 1 |
