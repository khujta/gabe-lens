...
| `gabe-universe.html` (NAMES · FRONTEND MARK pills) | under ENTITY MODEL (all three pills carry ICONS since 2026-09-07 — the word, the rule and the rendered mark ride the hover title): NAMES picks the strategy a cluster's words come from (domain · table · class · path · action · config · both — every candidate was emitted once as `names{}`; `row.name` is the domain strategy) and FRONTEND MARK the convention (case · prefix · suffix · bracket · glyph · tint · none — the emitted form templates, each button drawing its actual mark); a switch RELABELS and never re-clusters; precedence URL `?naming=`/`?feconv=` > localStorage `gabe:universe:naming`/`feconv` > the feed's default (`c4.models.naming` from `center.config.json#naming`) > built-in (domain · case); `__uniIsFeEnt` reads the registry + the `fe·` key, never a label | — |
| `gabe-universe.html` (the ENTITY MODEL pill) | the 3D station's top-right config carries a four-position pill claim · seeded · derived · proposed (entity models, 2026-09-06): it reads `window.GABE_C4.models` (+ `GABE_LEVELS.models` for the function layer) and SWITCHES A VIEW — `__uniSetModel` applies per-piece home deltas over the claim, never derives, never re-homes; `?model=<view>` picks it first, `gabe:universe:model` remembers it; `docs/site/center/entities.draft.json` (the drafter's projection) is loaded by NO page | — |
