/* three-kit.js — the INSTANCED render kit the Gabe Universe station draws with under ?render=instanced (lifted from the graph-renderer lab,
   docs/design/graph-renderers/lab/three-kit.js — the lab's copy stays the lab's; this one is the shell asset every center carries). Reads window.THREE
...
(function () {
  ...
  TK.sphereLayer = function (n, color, op, r, seg, wire) {
    var m = instanced(new T.SphereGeometry(r || 6.2, seg || 12, seg ? Math.max(6, seg - 4) : 9), new T.MeshBasicMaterial({ color: color, transparent: true, opacity: op, depthWrite: false, wireframe: !!wire, side: wire ? T.FrontSide : T.DoubleSide }), n); m.name = wire ? 'sphere-rims' : 'spheres';
  ...
  function quadLayer(tex, n, uvs, sizes, opacity) {
    ...
    var mat = atlasMaterial(tex); if (opacity != null) mat.opacity = opacity; var m = instanced(q, mat, n); m.renderOrder = 5; m.name = 'atlas';
    return { mesh: m, set: function (i, x, y, z) { setAt(m, i, x, y, z, 1); }, hide: function (i) { TK.hide(m, i); }, flush: function () { m.instanceMatrix.needsUpdate = true; if (m.instanceColor) m.instanceColor.needsUpdate = true; },
      setColor: function (i, c) { tmpC.set(c); m.setColorAt(i, tmpC); },   /* a WHITE atlas glyph tinted per instance (the station's billboard icon: white lucide strokes × the kind/heat/method colour) */
      setSize: function (i, w, h) { sizes[i * 2] = w; sizes[i * 2 + 1] = h; q.attributes.aSize.needsUpdate = true; } };
  }
  TK.quadLayer = quadLayer;
  /* an ATLAS of ready canvases (the station's per-kind billboard textures): one cell per entry, uv(i) per cell */
  TK.imageAtlas = function (images, S) {   // images: [{key, image}] (any drawable) → {tex, uv(i), index{key→i}}
    S = S || 128; var COLS = Math.max(1, Math.min(16, images.length)), ROWS = Math.max(1, Math.ceil(images.length / COLS)), cv = document.createElement('canvas'); cv.width = S * COLS; cv.height = S * ROWS; var c = cv.getContext('2d'), index = {};
    images.forEach(function (e, k) { var x = (k % COLS) * S, y = ((k / COLS) | 0) * S; index[e.key] = k; try { c.drawImage(e.image, x, y, S, S); } catch (err) {} });
    var tex = new T.CanvasTexture(cv); tex.colorSpace = T.SRGBColorSpace;
    return { tex: tex, index: index, uv: function (k) { return [(k % COLS) / COLS, 1 - (((k / COLS) | 0) + 1) / ROWS, 1 / COLS, 1 / ROWS]; } };
  };
  /* one quad per node over an image atlas, tinted per instance — the station's icon layer */
  TK.iconLayer = function (nodes, atlas, keyOf, size) {
    var n = nodes.length, uvs = new Float32Array(n * 4), sizes = new Float32Array(n * 2), sz = size || 10;
    nodes.forEach(function (nd, i) { var k = atlas.index[keyOf(nd)]; if (k == null) k = 0; var u = atlas.uv(k); uvs[i * 4] = u[0]; uvs[i * 4 + 1] = u[1]; uvs[i * 4 + 2] = u[2]; uvs[i * 4 + 3] = u[3]; sizes[i * 2] = sz; sizes[i * 2 + 1] = sz; });
    var L = quadLayer(atlas.tex, n, uvs, sizes); L.mesh.name = 'icons'; return L;
  };
  ...
  TK.badgeAtlas = function (entries, draw) {   // entries: [{family, value}] → {tex, uv(i)}; draw(ctx, x, y, S, entry) overrides the disc+letters (the station passes its own __badgeGlyph)
    var S = draw ? 128 : 64, COLS = 8, ROWS = Math.max(1, Math.ceil(entries.length / COLS)), cv = document.createElement('canvas'); cv.width = S * COLS; cv.height = S * ROWS; var c = cv.getContext('2d');
    entries.forEach(function (e, k) { var x = (k % COLS) * S, y = ((k / COLS) | 0) * S; if (draw) { try { draw(c, x, y, S, e); } catch (err) {} return; } c.fillStyle = TK.badgeColor(e.family, e.value); c.beginPath(); c.arc(x + S / 2, y + S / 2, S * 0.44, 0, 6.2832); c.fill();
  ...
  TK.badgeLayer = function (nodes, slotFn, size, draw, opacity) {   // slotFn(n) → [{family,value}|null, {family,value}|null]; one instance per (node, slot) that has a badge; draw = a custom glyph drawer
    ...
    var atlas = TK.badgeAtlas(entries, draw), uvs = new Float32Array(items.length * 4), sizes = new Float32Array(items.length * 2), sz = size || 6.5;
    ...
    var L = quadLayer(atlas.tex, items.length, uvs, sizes, opacity); L.mesh.name = 'badges'; L.items = items; L.atlas = atlas;
  ...
  TK.wireLayers = function (links, feed, colorOf) {   // colorOf(link, endIndex) → hex; returns {layers{kind}, update(posOf), setVisible(li,on)}
    ...
    Object.keys(byKind).forEach(function (kind) { var idx = byKind[kind], cfg = feed.conn[kind] || feed.conn.calls, n = idx.length;
      ...
      layers[kind] = { mesh: ls, pos: pos, idx: idx, dashed: cfg.style !== 'solid', cfg: cfg }; });
