...
(async()=>{
  ...
  const storeCheck = await p.evaluate(() => {
    ...
    return { journey:j.name, storeCols, storeFlagOn, litStore, iconInline, titleHot, cellClean, row0Lit, anchorKind, anchorKind3, lgbiOk, feStart, hookRoles, drafts:drafts.length, draftWalk, draftGrp, draftLeveled:drafts.every(x=>x.level>=1&&x.level<=3), draftNamed:drafts.every(x=>/^(Look at|Add|Edit|Remove|Manage) /.test(x.name||'')), viewType, viewCol, viewIcon };
  }).catch(e=>({err:String(e)}));
  await b.close();
